...
class OrderRepository:
    ...
    async def save(self, order: Order) -> None:
        ...
        for item in order.items:
            # subtotal is a GENERATED ALWAYS AS column — never insert it explicitly
            await self.session.execute(
                text("""
    ...
    async def find_by_id(self, order_id: uuid.UUID) -> Optional[Order]:
        ...
        order.items = [self._row_to_item(r) for r in items_result.fetchall()]
        ...
        order.status_history = [
            self._row_to_history(r) for r in history_result.fetchall()
        ]
