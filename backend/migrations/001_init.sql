-- ============================================
-- Схема базы данных маркетплейса (lab_03)
-- ============================================
...
-- ============================================
-- Позиции заказа
-- Subtotal вычисляется СУБД автоматически.
-- Seed-скрипт (01_seed_100k.sql) не передаёт subtotal явно — это корректно.
-- ============================================
...
-- Триггер срабатывает только при ПЕРЕХОДЕ статуса на 'paid',
-- а не при обновлении других полей (total_amount и т.д.).
-- ============================================
CREATE OR REPLACE FUNCTION check_order_not_already_paid()
RETURNS TRIGGER AS $$
BEGIN
    -- Проверяем только смену статуса на 'paid' (переход, не повторная установка)
